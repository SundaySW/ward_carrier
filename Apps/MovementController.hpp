...
public:
    ...
    constexpr void setADCSensorsVars(T(&adc_values_in)[N]){
        adc_values = adc_values_in;
        adc_values_size = N;
    }

    template<typename T, uint8_t N>
    constexpr void setHALLSensorsVars(T(&hall_l)[N], T(&hall_r)[N]){
    ...
    inline void checkHALLSensors(){
        ...
        for(uint8_t i = 0; i < hall_values_size; i++){
            AveragePulseL += *l_hall_values++;
            AveragePulseR += *r_hall_values++;
        }
        AveragePulseL /= hall_values_size;
        AveragePulseR /= hall_values_size;
        uint16_t diff = AveragePulseL > AveragePulseR ? (AveragePulseL - AveragePulseR) : (AveragePulseR - AveragePulseL);
...
protected:
private:
    uint16_t* l_hall_values;
    uint16_t* r_hall_values;
